# -*- coding: utf-8 -*-
"""
Created on Wed Apr  3 09:43:16 2019
...
class DDFContSpecModulator(BaseModulator):
    ...
    def __init__(self,
        ...
        '''print("Size of the xi grid is",self._xi.size)
        'print("Size of the CARRIER centers is",self._carrier_centers.size)'''
...
#        print("XI is",self._XI)
#        print("t resolution is",self._t[1]-self._t[0])
#        print("Size of t is",self._t.size)
#        print("first element of t is",self._t[0])
#        print("T window is",self._Tnew)
        #print("Size of the xi grid is",self._xi.size)
        #print("first element of the xi grid is",self._xi[0])
        #print("last element of the xi grid is",self._xi[8191])
        #print("resolution of the xi grid is",self._xi[1]-self._xi[0])
        #print("Size of the carrier centers is",self._carrier_centers.size)  
        #print("first element of the carrier centers is",self._carrier_centers[0])  
        #print("resolution of the carrier centers is",self._carrier_centers[1]-self._carrier_centers[0])  '''
        ...
        for n in range(0, nc):
            ...
            #plt.xlabel('nonlinear freq')
            #plt.ylabel('amplitude')
            #plt.title('subcarrier shape')
            # xi min and xi max are boundaries of XI
        ...
        nfspec.cont *= np.exp(-2.0j*self._xi**2*self._normalized_distance*0.413*(self._percent_precompensation/100))
        #print('nfspec type is',type(nfspec.cont))
    ...
    def demodulate(self, q):
        ...
        assert np.size(q) == self.n_samples   #1024
        from scipy import signal
        DO = self._O * self.n_samples        # 4*1024
        ...
        D_z_avg = 0.414              # Normalized dispersion averaged over span 
        
        # The factor ideally should be 0.413 for 80 km and -25.491e-27 starting dispersion, but
        # its optimum value changes with step size of split step Fourier method (i.e. depends on accuracy)
        # As checked by EVM performance, its value found as 0.4134 for 1000 steps and 0.414 for 500 steps.       
        nfspec.cont *= np.exp(-2.0j*self._xi**2*self._normalized_distance*D_z_avg
                               *(1 - self._percent_precompensation/100))
        #for i in range(0,self._M):
        #    nfspec.cont[i] *= np.exp(+1.0j*2*self._xi[i]*((2.5e-9/4e-10)%(2*np.pi)))
